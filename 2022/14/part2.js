...
// x-dim range and maximal y cordinate (which is a line
// of rock below in part 2 of the puzzle)
class Cave {
	constructor(segs, y_max, x_min, x_max) {
		this.y_max = y_max + 2
		//this.x_min = x_min
		//this.x_max = x_max
		this.x_min = 500 - y_max - 2 - 1
		this.x_max = 500 + y_max + 2 + 1
		this.source_x = 500 - this.x_min
		this.count = 0
		// Have the units of sand reached the source yet?
		this.done = false
		this.grid = []
		this.segs = segs
		this.init_grid()
	...
	init_grid() {
		let x_range = (this.x_max) - (this.x_min) + 1
		//let x_range = (this.y_max + 2) * 2 + 1
		let y_range = this.y_max + 1
		for (let y = 0; y < y_range; y++)
			this.grid.push(Array(x_range).fill(0))
	...
	drop() {
		...
		// if uniots of sand have not reached source yet
		// mark position and increase count
		if (y > 1) {
			this.grid[y-1][x-1] = 2
			this.count += 1
		} else {
			// source is reached, we're done
			this.count += 1
			this.done = true
		}
	}
...
for (let r of arr) {
	...
	for (let k = 0; k < spl.length-1; k++) {
		...
		//if (y_min === undefined) y_min = p1[0] < p2[0] ? p1[0] : p2[0]
