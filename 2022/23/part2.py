...
class Crater:
    ...
    def __init__(self, elves):
        ...
        #self.min_y, self.max_y, self.min_x, self.max_x = self.coords_range()
        self.shift_zero()
    ...
    def step(self):
        ...
        self.round_count += 1
        self.propose()
        # if there is no proposal anymore
        # we are finished
        if len(self.valid_props) == 0:
            no_move = True
        if not no_move:
            self.move()
            self.shift_zero()
...
if __name__ == '__main__':
    ...
    # in part 2 movements continue until the step function
    # signals there is no proposal anymore.
    done = False
    while not done:
        done = crater.step()

    print(crater.round_count)
