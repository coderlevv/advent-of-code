...
def formula(name="root"):
    if name == "humn":
        return "x"
    if monkeys[name].number != "":
        return monkeys[name].number
    op1 = formula(monkeys[name].op1)
    op2 = formula(monkeys[name].op2)
    if name == "root":
        func = "="
    else:
        func = monkeys[name].func
    if name == "root":
        return f"{op1}{func}{op2}"
    else:
        return f"({op1}{func}{op2})"

pvgq_expr = formula("pvgq")
ngpl_val = compute("ngpl")
expr = pvgq_expr + f"-{ngpl_val}"
x = symbols("x")
sol = solve(expr)
print(round(sol[0]))
